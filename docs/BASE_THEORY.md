...
1.  **Asset Price ($S_t$)**: $dS_t = r S_t dt + \sqrt{V_t} S_t dW^s_t$
2.  **Variance ($V_t$)**: $dV_t = \kappa(\theta - V_t)dt + \xi\sqrt{V_t}dW^v_t$
...
1.  **Asset Price ($S_t$)**: $dS_t = r S_t dt + \sqrt{V_t} S_t dB_t$
2.  **Variance ($V_t$)**: $V_t = V_0 \exp\left(\eta Y_t - \frac{1}{2}\eta^2 t^{2H}\right)$ where $Y_t = \int_0^t (t-s)^{H-1/2} \, dW^v_s$
...
1.  The path is **augmented** with the time dimension to create a new path $Y_t \in \mathbb{R}^{d+1}$, where the mathematical definition is:
    $$Y_t = (t, X_t)$$
2.  A continuous, **piecewise linear path** is constructed by connecting the points. For any time $s$ within a simulation interval $[t_i, t_{i+1}]$, the path is formally defined by the linear interpolation formula:
